...
//		int num = Integer.parseInt(str);
		...
		while (true) {
			System.out.println("                       モード選択 \n\n【(1)売上確認】 【(2)売上リセット】 【(3)商品在庫】  【(4)釣銭在庫】\n");
			String str = br.readLine();
			String ch = str.intern();
			if (ch == "1") {
				Kanri.Ukanri();//For Uriage kakuninn
				// break;
			} else if (ch == "3") {
				Kanri.Skanri(); //For Stock Management
				// break;
			} else if (ch == "4") {
				Kanri.Mkanri();//For Churisen Management
				// break;
			}else if (ch == "2") {
				DbCon.UUreset();//For Sales reset
				//break;
			} else if (ch == "exit") {
				System.out.println("\n       【商品一覧表】\n");
				break;
...
//Connect Database and Add the stocks//商品補充
